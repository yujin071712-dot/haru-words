...
const CalendarView: React.FC<{ 
  ...
  for (let d = 1; d <= days; d++) {
    ...
    dayElements.push(
      <button
        ...
        className={`relative h-14 md:h-20 border border-slate-100 flex flex-col items-center justify-center rounded-lg transition-colors ${
  ...
  return (
    <div className="bg-white rounded-2xl p-6 shadow-sm max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <p className="mt-4 text-sm text-slate-500 text-center italic">공부한 날짜를 클릭하면 해당 날짜의 단어를 테스트할 수 있습니다.</p>
    </div>
  );
};
...
const TestView: React.FC<{ 
  ...
  const handleNext = () => {
    if (userInput.trim() && currentWord.meaning.includes(userInput.trim())) {
      setScore(s => s + 1);
    }
  ...
  if (isFinished) {
    return (
      <div className="flex flex-col items-center justify-center space-y-6 text-center py-10 animate-in fade-in zoom-in">
        <div className="w-24 h-24 bg-indigo-100 text-indigo-600 rounded-full flex items-center justify-center mb-4">
          <BrainCircuit size={48} />
        </div>
        <h2 className="text-3xl font-bold">테스트 완료!</h2>
  ...
  return (
    <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 shadow-xl border border-slate-100 animate-in fade-in scale-95 duration-300">
      ...
      <div className="text-center mb-10">
        <h3 className="text-6xl font-bold japanese-text mb-4 text-slate-800 tracking-tight">{currentWord.kanji || currentWord.hiragana}</h3>
        <p className="text-slate-400 text-lg japanese-text">{currentWord.hiragana}</p>
      </div>

      <div className="space-y-4">
        <label className="block text-sm font-bold text-slate-500 uppercase tracking-wider">한국어 뜻을 입력하세요</label>
        <input 
          type="text" 
          value={userInput}
          ...
          disabled={showResult}
          placeholder="여기에 입력..."
          className="w-full px-6 py-4 rounded-2xl border-2 border-slate-100 focus:border-indigo-400 focus:outline-none text-lg transition-all"
      ...
      {showResult && (
        <div className={`mt-6 p-4 rounded-xl text-center font-bold animate-in slide-in-from-bottom-2 ${
          currentWord.meaning.includes(userInput.trim()) ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
        }`}>
...
const KanaTestView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="max-w-xl mx-auto bg-white rounded-3xl p-8 shadow-xl border border-slate-100 animate-in fade-in scale-95">
      ...
      <div className="text-center mb-12">
        <h3 className="text-9xl font-black japanese-text text-slate-800">{currentQ[0]}</h3>
      </div>
      <div className="space-y-4">
        <input 
          type="text" 
          ...
          onChange={(e) => setUserInput(e.target.value)}
          disabled={showResult}
          placeholder="발음을 한글로 입력 (예: 아)"
          className="w-full px-6 py-5 rounded-2xl border-2 border-slate-100 focus:border-indigo-400 focus:outline-none text-xl text-center transition-all"
          onKeyDown={(e) => e.key === 'Enter' && handleNext()}
          autoFocus
        />
        {showResult && (
          <div className={`p-4 rounded-xl text-center font-bold animate-in slide-in-from-bottom-2 ${
            userInput.trim() === currentQ[1] ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
          }`}>
            정답: {currentQ[1]}
          </div>
        )}
  ...
  const startDailyStudy = async () => {
    ...
    if (history[today]) {
      ...
      try {
        ...
        const newSession: DailyStudySession = {
          date: today,
          words,
          completed: false,
        };
  ...
  const renderContent = () => {
    ...
    switch (view) {
      case 'home':
        ...
        const totalStudiedDates = Object.keys(history).length;
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[32px] p-8 text-white shadow-xl flex flex-col justify-between">
                ...
                <button 
                  ...
                  className="mt-12 bg-white text-indigo-700 py-5 rounded-2xl font-black text-xl hover:bg-slate-50 transition-all shadow-xl active:scale-95"
                >
